...
class game {
    ...
    private $closed_cards = NULL;             //list of closed crads
    ...
    public  function game_starts ($player_a, $player_b) {
        ...
        foreach (array_rand($this->all_cards, 7) as $k) {
        ...
        foreach (array_rand($t_cards, 7) as $k) {
    ...
    private function game_init_all_cards() {
        ...
        //creating "+" cards
        foreach(array('red', 'blue', 'yellow', 'green') as $color) {
            $card = new card("", $color, "plus", "");
            array_push($this->all_cards, $card);
        }
        //creating "<==>" cards
        foreach(array('red', 'blue', 'yellow', 'green') as $color) {
            $card = new card("", $color, "change_dir", "");
            array_push($this->all_cards, $card);
        }
        //creating "stop" cards
        foreach(array('red', 'blue', 'yellow', 'green') as $color) {
            $card = new card("", $color, "stop", "");
            array_push($this->all_cards, $card);
        }
        //creating "change col" cards
        foreach(array('red', 'blue', 'yellow', 'green') as $color) {
            $card = new card("", $color, "change_col", "");
            array_push($this->all_cards, $card);
        }
        //creating TAKI cards
        foreach(array('red', 'blue', 'yellow', 'green') as $color) {
            $card = new card("", $color, "taki", "");
            array_push($this->all_cards, $card);
        }
        //creating "change cards" cards
        foreach(array('red', 'blue') as $color) {
            $card = new card("", $color, "change_cards", "");
            array_push($this->all_cards, $card);
        }
    }
    ...
    private function game_ends () {
        ...
        $average_num_of_cards_per_game= ($this->highest_num_cards_a+$average_num_of_cards_per_game)/($num_of_games -1);
        ...
        $average_num_of_cards_per_game= ($this->highest_num_cards_b+$average_num_of_cards_per_game)/($num_of_games -1);
        $this->model->tm_update_player($username,$user_password,$nick_name,$num_of_games,$num_of_wins,$num_of_loses,$average_num_of_cards_per_game);
    }
